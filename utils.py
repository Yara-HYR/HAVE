...
class ReplayBuffer(object):
    ...
    def sample_ctmr(self):
        ...
        # non_masked = np.zeros((self.mtm_bsz, self.mtm_length), dtype=np.bool_)

        # import pdb
        # pdb.set_trace()
        # obses = self.random_obs(obses_label, non_masked)
        # non_masked = torch.as_tensor(non_masked, device=self.device)
        # # obses_label = augmentations.random_crop_2(obses_label, self.image_size)
        # # obses = augmentations.random_crop_2(obses, self.image_size)

        # obses = torch.as_tensor(obses, device=self.device).float()
        # obses_label = torch.as_tensor(obses_label, device=self.device).float()

        # # return (*self.sample(), dict(obses=obses, obses_label=obses_label,
        # #             non_masked=non_masked ))

        return (dict(obses=obses, actions=actions, curr_rewards=curr_rewards,rewards= rewards, next_obses=next_obses, not_dones=not_dones ))
